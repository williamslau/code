...
import { List, Spin, Avatar, Table, Input, Popconfirm, Button, Form, message, Select, Col, Row, Modal, Card } from 'antd';
...
export default class Article extends Component {
    ...
    getList = () => {
        ...
        articleService.list({ current: this.state.pagination.current, keyword: this.state.keyowrd }).then(res => {
    ...
    commentOk = () => {
        let comment = this.commentForm.props.form.getFieldsValue();//{content:'xx'}
    ...
    render() {
        //标题 内容 分类 阅读量 添加时间 评论数 操作 {title,content}
        let columns = [
            {
                title: '标题',
                dataIndex: 'title',
